...
def main():
    ...
    model_path = "./pretrained-models/bert-base-chinese/"
    data_path = "./dataset/cnews/"
    batch_size = 4
    ...
    model.load_state_dict(
        torch.load("models/best_model.pkl", map_location=torch.device(device))
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        report = metrics.classification_report(
            true_labels,
            pred_labels,
            labels=valid_dataset.labels_id,
        ...
        # f1 用来判断最优模型
        f1 = metrics.f1_score(
            true_labels, pred_labels, labels=valid_dataset.labels_id, average="micro"
        )
